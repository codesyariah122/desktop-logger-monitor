...
class EmailDialog(QDialog):
    ...
    def initUI(self):
        ...
        # Heading dengan font size lebih besar dan jarak bawah
...
class ActivityMonitorApp(QWidget):
    ...
    def get_user_data_from_api(self):
        ...
        try:
            response = requests.get(f'http://localhost:9091/api/get-user-data?email={self.user_email}')
            if response.status_code == 200:
    ...
    def display_user_data(self, user_data):
        ...
        # Misalnya, menampilkan nama dan informasi lain dari data pengguna
        if 'name' in user_data:
            self.user_name_label = QLabel(f"Name: {user_data['name']}", self)
            self.user_name_label.setStyleSheet("font-size: 14px; margin-top: 10px;")
            self.layout().addWidget(self.user_name_label)

        if 'role' in user_data:
            self.user_role_label = QLabel(f"Role: {user_data['role']}", self)
            self.user_role_label.setStyleSheet("font-size: 14px; margin-top: 10px;")
            self.layout().addWidget(self.user_role_label)

        if 'last_login' in user_data:
            self.last_login_label = QLabel(f"Last Login: {user_data['last_login']}", self)
            self.last_login_label.setStyleSheet("font-size: 14px; margin-top: 10px;")
            self.layout().addWidget(self.last_login_label)
